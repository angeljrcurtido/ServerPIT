...
const bookSchema = new mongoose.Schema({
  ...
  descripcion: String,
  cantidad: Number,
  precioAlquiler: Number, // Agregado el campo precioAlquiler
  alquilados: [
    {
      nombreLibro: String,
      codigolibro: String,
      ...
      totalPago: Number, // Nuevo campo para registrar el total del alquiler
      estado: { type: String, default: 'prestado' },
      multa: { type: Number, default: 0 }, // Nuevo campo para la multa
    },
  ],
});
...
app.post('/libros', async (req, res) => {
  try {
    const { titulo, codigodelibro, categoria, imagen, autor, subcategoria, descripcion, cantidad, precioAlquiler } = req.body;
    ...
    const newBook = new Book({
      ...
      descripcion,
      cantidad,
      precioAlquiler, // Asegúrate de que el precio de alquiler se guarde
    });
...
app.post('/libros/alquilar', async (req, res) => {
  try {
    ...
    // Calcular el total a pagar por el alquiler
    const totalPago = book.precioAlquiler * cantidadAlquilada;
    ...
    const newRental = {
      ...
      fechaDevolucion,
      cantidadAlquilada,
      totalPago, // Registrar el total del alquiler
    };
...
app.patch('/libros/devolver/:alquilerId', async (req, res) => {
  try {
    ...
    const { multa } = req.body; // Extraer multa del cuerpo de la solicitud
    ...
    alquiler.multa = multa || 0; // Actualizar el campo multa con el valor proporcionado
